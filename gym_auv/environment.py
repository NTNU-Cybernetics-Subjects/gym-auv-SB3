...
class BaseEnvironment(gym.Env, ABC):
    ...
    def __init__(self, env_config, test_mode=False, render_mode="2d", verbose=False):
        ...
        self._perception_space = gym.spaces.Box(
            low=0, high=1, shape=(1, self._n_sensors), dtype=np.float32
        )
    ...
    def store_statistics_to_file(self, path):
        ...
        if not self.history:
            print(
                "DEBUG: environment.py: store_statistics_to_file(): self.history is empty, skipping..."
            )
            return
        ...
        if not os.path.exists(path_history):
            ...
            trajectory_table = f.create_table(
                group, "trajectory", TrajectoryLog, "Trajectories"
            )
            ...
            table_row["path_taken"][: self.last_episode["path_taken"].shape[0], :] = (
                self.last_episode["path_taken"]
            )
            table_row["obstacles"][: self.last_episode["obstacles"].shape[0], :] = (
                self.last_episode["obstacles"]
                if self.last_episode["obstacles"]
                else None
            )
            table_row.append()
