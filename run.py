...
def play_scenario(env, recorded_env, args, agent=None):
    ...
    try:
        while True:
                ...
                if args.save_snapshots and not done:
                    if t_steps % 50 == 0:
                        ...
                        for size in (100, 200):#, 300, 400, 500):
                            gym_auv.reporting.plot_trajectory( # FIXME: figure_folder
            ...
            gym_auv.reporting.plot_trajectory(figure_folder, env, fig_dir='logs/play_results/') # FIXME: figure_folder does not exists in this scope
...
def play_mode(envconfig, model, env_id):
...
def enjoy_mode(envconfig, model, env_id, EXPERIMENT_ID):
...
def train_mode(envconfig, model, env_id, EXPERIMENT_ID, NUM_CPU, envconfig_string):
...
def test_mode(envconfig, model, env_id, EXPERIMENT_ID, NUM_CPU):
...
def main(args):
    ...
    if args.mode == 'play':
        play_mode(envconfig, model, env_id)

    elif (args.mode == 'enjoy'):
        enjoy_mode(envconfig, model, env_id, EXPERIMENT_ID)

    elif (args.mode == 'train'):
        train_mode(envconfig, model, env_id, EXPERIMENT_ID, NUM_CPU, envconfig_string)

    elif args.mode == 'test':
        test_mode(envconfig, model, env_id, EXPERIMENT_ID, NUM_CPU)
